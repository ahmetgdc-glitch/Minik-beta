...
// Exercise the generated worker against the actual distribution files at both
// GitHub user-site and repository-site paths. No browser or network is required.
...
for (const match of html.matchAll(/(?:src|href)="([^"#]+)"/g)) {
  assert.ok(
    !match[1].startsWith("/"),
    `Root-relative build asset: ${match[1]}`,
  );
...
for (const scope of [
  ...
  async function fetchFile(request) {
    ...
    if (serverFailure) {
      return {
        ...
        status: 503,
        type: "basic",
        body: Buffer.from("temporary server error"),
        clone() { return this; },
      };
    }
  ...
  const caches = {
    async open(name) {
      ...
      return {
        async addAll(files) {
          for (const file of files) store.set(key(file), await fetchFile(file));
        },
        async match(request) {
  ...
  const active = [...stores.entries()].find(([name]) =>
  ...
  assert.match(
    degradedPage.body.toString(),
    /MINIK/,
    "A temporary 5xx navigation must fall back to the cached app shell",
  );
  serverFailure = false;
  offline = true;
