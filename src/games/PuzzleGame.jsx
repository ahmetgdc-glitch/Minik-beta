...
function puzzleLayout(difficulty) {
  const count = difficultyProfile(difficulty).puzzlePieces;
  if (count >= 9) return { count: 9, cols: 3, rows: 3 };
  if (count >= 6) return { count: 6, cols: 3, rows: 2 };
...
export default function PuzzleGame({
  ...
  const guideOpacity = hint >= 3 ? .30 : hint >= 2 ? .20 : baseGuide;

  return (
    <section className="puzzle-layout real-puzzle" data-difficulty={profile.id} aria-disabled={controlsDisabled || undefined} ref={placement.boardRef}>
      <header className="puzzle-topbar">
        <div className="puzzle-reference"><Visual item={target} lang={lang} photos={settings.photos} /><span>{lang === "tr" ? "Örnek" : "Vorbild"}</span></div>
        <div className="puzzle-progress" role="status" aria-label={progressLabel}><strong>{progressLabel}</strong><div>{Array.from({ length: count }, (_, index) => <i key={index} className={placed.includes(index) ? "done" : ""}/>)}</div></div>
      </header>
      <div className="puzzle-workspace">
        <div className={`real-puzzle-board grid-${cols}x${rows}`} style={{ "--puzzle-image": `url("${url}")`, "--guide-opacity": guideOpacity, "--puzzle-cols": cols, "--puzzle-rows": rows }} aria-label={lang === "tr" ? "Puzzle tahtası" : "Puzzlebrett"}>
          {Array.from({ length: count }, (_, index) => {
            const filled = placed.includes(index);
            return <button key={index} type="button" data-drop-id={slotId(index)} disabled={controlsDisabled || filled} className={`puzzle-slot puzzle-piece shape-${index % 4} ${filled ? "filled" : ""} ${hint >= 2 && selectedIndex === index ? "hint-target" : ""}`} style={filled ? pieceStyle(index) : undefined} onClick={() => drop(selected, slotId(index))} aria-label={`${lang === "tr" ? "Puzzle yeri" : "Puzzleplatz"} ${index + 1}`}>{!filled && hint >= 3 && <span>{index + 1}</span>}{filled && <span className="puzzle-piece-check">✓</span>}</button>;
          })}
        </div>
        <div className={`puzzle-tray pieces-${count}`} aria-label={lang === "tr" ? "Puzzle parçaları" : "Puzzleteile"}>
          ...
          <div className="puzzle-tray-grid">
            {tray.map((index) => {
              ...
              return <button key={index} type="button" disabled={controlsDisabled || done} className={`puzzle-source puzzle-piece shape-${index % 4} ${done ? "placed" : ""} ${selected === id ? "selected" : ""}`} style={pieceStyle(index)} aria-label={`${lang === "tr" ? "Parça" : "Puzzleteil"} ${index + 1}`} aria-pressed={selected === id} {...placement.sourceProps(id)} />;
            })}
          </div>
        </div>
      </div>
      {placement.drag && (() => { const index = parseIndex(placement.drag.id, "piece-"); if (index < 0) return null; return <div className={`puzzle-drag-ghost puzzle-piece shape-${index % 4}`} style={{ ...pieceStyle(index), left: placement.drag.x, top: placement.drag.y }} aria-hidden="true"/>; })()}
    </section>
  );
}
