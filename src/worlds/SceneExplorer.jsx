  ...
  return (
    ...
    >
      <img
        ...
        src={assetUrl(`assets/scenes/${scene}.webp`)}
        alt=""
        draggable="false"
      />
      <WorldScenery worldId={worldId} />
      <div
        className="discovery-viewport"
        ref={pager.scrollRef}
        onScroll={pager.onScroll}
        onKeyDown={(event) => {
          if (!controlsDisabled) pager.onKeyDown(event);
        }}
      >
        {items.map((item, index) => {
          ...
          return (
            ...
            >
              <button
                className={`discovery-object ${found.includes(item.id) ? "discovered" : ""} ${speaking ? "speaking" : ""} ${hint >= 2 && !found.includes(item.id) ? "hint-target" : ""}`}
      ...
      <button
        className="scene-round-button discovery-prev"
        disabled={pager.index === 0 || controlsDisabled}
        onClick={() => pager.go(pager.index - 1)}
        aria-label={t("Vorheriges Bild", "Önceki resim")}
      >
        <ChevronLeft size={29} />
      </button>
      <button
        className="scene-round-button discovery-next"
        disabled={pager.index === items.length - 1 || controlsDisabled}
        onClick={() => pager.go(pager.index + 1)}
        aria-label={t("Nächstes Bild", "Sonraki resim")}
      >
        <ChevronRight size={29} />
      </button>
      <div className="discovery-footer">
        <button
          className="scene-mino"
          onClick={onMino}
          disabled={controlsDisabled}
          aria-label={t("Mino zuhören", "Mino’yu dinle")}
        >
          <MinoAvatar outfit={outfit} />
        </button>
        <span className="discovery-position" aria-live="polite">
