  ...
  return (
    <>
      ...
      <div className="treasure-panel">
        ...
        <button className="primary" disabled={!available.length} onClick={claim}>
          <Gift size={21} />
          {lang === "tr" ? "Aç" : "Öffnen"}
        </button>
      </div>

      <section className="treasure-reef" aria-labelledby="treasure-reef-title">
        <div className="treasure-reef-heading">
          <h2 id="treasure-reef-title">
            {lang === "tr" ? "Hazine resifin" : "Dein Schatzriff"}
          </h2>
          <span>
            {progress.inventory.length} / {rewards.length} {lang === "tr" ? "hazine" : "Schätze"}
          </span>
        </div>
        <div className="treasure-objects">
          {rewards.map((r) => {
            const owned = progress.inventory.includes(r.id);
            return (
              <button
                className={`treasure-object ${owned ? "owned" : ""} ${equipped(r.id) ? "equipped" : ""}`}
                key={r.id}
                disabled={!owned}
                onClick={() => dispatch({ type: "equip", id: r.id })}
                aria-pressed={owned ? equipped(r.id) : undefined}
              >
                <Art name={r.asset} />
                <h3>{r[lang]}</h3>
                <span>
      ...
      <section className="outfit-parade" aria-labelledby="outfit-title">
        <div className="outfit-parade-heading">
          <div>
            <span className="eyebrow">
              {lang === "tr" ? "Mino’yu giydir" : "Mino anziehen"}
            </span>
            <h2 id="outfit-title">
              {lang === "tr" ? "Mino kıyafetleri" : "Minos Outfits"}
            </h2>
          </div>
          <p>
            {outfitNext
              ? lang === "tr"
                ? `${outfitNext.stars - progress.stars} yıldız sonra yeni kıyafet`
                : `Noch ${outfitNext.stars - progress.stars} Sterne bis zum nächsten Outfit`
              : lang === "tr"
                ? "Bütün kıyafetler açıldı!"
                : "Alle Outfits sind freigeschaltet!"}
          </p>
        </div>
        <div className="outfit-parade-track">
          {minoOutfits.map((o) => {
            ...
            return (
              <button
                key={o.id}
                className={`outfit-character ${unlocked ? "unlocked" : "locked"} ${selected ? "selected" : ""}`}
                disabled={!unlocked}
