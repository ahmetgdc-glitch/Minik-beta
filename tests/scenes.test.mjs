...
test("the atlas reaches every existing world exactly once without changing IDs", () => {
  ...
  assert.ok(
    pages.every((page) => page.ids.length >= 1 && page.ids.length <= 2),
  );
...
test("filtered atlases never offer unavailable worlds or empty islands", () => {
  for (const world of worlds)
    assert.deepEqual(
      atlasPages([world]).flatMap((p) => p.ids),
      [world.id],
    );
...
test("scene paging rounds native scroll positions and clamps Safari overscroll", () => {
  ...
  for (const width of [0, -1, NaN, Infinity])
    assert.equal(sceneIndex(400, width, 5), 0);
...
test("discovery advances once per real item despite rapid duplicate taps", () => {
  ...
  let found = [];
  for (const id of [ids[0], ids[0], "unknown", ids[1], ids[1]])
    found = addDiscovery(found, id, ids);
...
test("all scene backgrounds exist locally within a compact offline budget", () => {
  ...
  for (const name of new Set([
    "archipelago",
    ...worlds.map((w) => sceneForWorld(w.id)),
  ])) {
    const file = new URL(
      `../public/assets/scenes/${name}.webp`,
      import.meta.url,
    );
...
test("every learning world has its own complete local landmark set", () => {
  assert.deepEqual(
    Object.keys(sceneDecorations).sort(),
    worlds.map((world) => world.id).sort(),
  );
  const signatures = new Set();
  for (const world of worlds) {
    const decorations = decorationsForWorld(world.id);
    assert.equal(
      decorations.length,
      3,
      `${world.id} should have three landmarks`,
    );
    assert.equal(
      new Set(decorations).size,
      decorations.length,
      `${world.id} landmarks should be distinct`,
    );
    signatures.add(decorations.join("|"));
    for (const asset of decorations) {
      const file = new URL(
        `../public/assets/illustrations/${asset}.svg`,
        import.meta.url,
      );
  ...
  assert.equal(
    signatures.size,
    worlds.length,
    "every world should have a recognisable landmark combination",
  );
  assert.deepEqual(decorationsForWorld("unknown"), []);
});
