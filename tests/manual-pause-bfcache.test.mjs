...
test("manual pause intent is set by pause control and cleared by explicit resume", () => {
  assert.match(source, /const pauseManually = useCallback\(\(\) => \{[\s\S]*manualPauseRef\.current\s*=\s*true/);
  assert.equal((source.match(/onClick=\{pauseManually\}/g) || []).length, 1, "the dedicated pause button should use the guarded manual pause helper");
